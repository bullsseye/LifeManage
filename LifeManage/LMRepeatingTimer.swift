...
class LMRepeatingTimer: NSObject {
        ...
        Alamofire.request(request).responseJSON { (response) in
            if (response.result.isSuccess) {
                ...
                if (urlString != "None" && self != nil && self!.delegate != nil) {
                    self!.delegate!.didReceiveRemoteNotificationToOrder(fromRepeatingTimer: self!, baseURLString: urlString)
                    self!.timer.suspend()
                }
            } else {
